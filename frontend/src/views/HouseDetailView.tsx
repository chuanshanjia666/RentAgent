...
import {
  Alert, Button, Descriptions, Empty, Input, message, Modal, Tag, Typography
} from 'antd'
...
export default function HouseDetailView() {
  ...
  const facilities = useMemo<string[]>(() => {
    const f = item?.house.facilities
    try {
  ...
  async function load() {
    ...
    setItem(d)
    const p = await http.get(`/houses/${id}/reviews`, { params: { size: 20 } })
    setReviews(p.list)
  }
  ...
  if (!item) return <div className="page"><p>加载中…</p></div>

  return (
    <div className="page">
      <Button type="text" onClick={() => history.back()} style={{ marginBottom: 10 }}>← 返回</Button>
      <div className="detail-layout">
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <Title level={3} style={{ marginTop: 16 }}>
            {item.house.title}
            <Tag color={item.house.status === 3 ? 'green' : 'blue'} style={{ verticalAlign: 'middle', marginLeft: 8 }}>
              {HOUSE_STATUS[item.house.status ?? 0]}
            </Tag>
          </Title>
          <Descriptions bordered size="small" column={3}>
            <Descriptions.Item label="小区">{item.house.community}</Descriptions.Item>
            <Descriptions.Item label="区域">{item.house.city} {item.house.district}</Descriptions.Item>
            ...
            <Descriptions.Item label="朝向/楼层">{item.house.orientation || '-'} / {item.house.floorDesc || '-'}</Descriptions.Item>
            ...
            <Descriptions.Item label="评分">
              {item.house.avgScore ? `⭐ ${item.house.avgScore}` : '暂无'}（{item.reviewCount} 条评价）
            </Descriptions.Item>
          </Descriptions>

          <h4 style={{ marginTop: 18 }}>设施</h4>
          {facilities.map(f => <Tag key={f} style={{ marginBottom: 8 }}>{f}</Tag>)}
          <h4>房源描述</h4>
          ...
          {reviews.map(r => (
            <div key={r.id} className="review-item">
              <b>{r.tenantName}</b>
              <span style={{ color: '#faad14', marginLeft: 8 }}>{'★'.repeat(r.houseScore ?? 0)}</span>
              <span style={{ color: '#909399', fontSize: 12, marginLeft: 8 }}>{fmtTime(r.createdAt)}</span>
        ...
        <div style={{ width: 300, flexShrink: 0 }}>
          <div className="price-card">
            <div className="house-rent" style={{ fontSize: 26 }}>
              {fmtMoney(item.house.rent)}<small> /月</small>
            </div>
            ...
            {isTenant ? (
              <>
                <Button type="primary" size="large" block onClick={() => setApptOpen(true)}>📅 预约看房</Button>
                <Button size="large" block style={{ margin: '10px 0 0' }} onClick={() => setContractOpen(true)}>
                  📝 发起在线签约
                </Button>
                <Button type="text" block style={{ marginTop: 10 }} onClick={toggleFav}>
                  {item.favorited ? '♥ 已收藏' : '♡ 收藏房源'}
                </Button>
                <Button type="text" size="small" block onClick={() => setReportOpen(true)}>举报该房源</Button>
              </>
      ...
      <Modal title="预约看房" open={apptOpen} onOk={submitAppt} onCancel={() => setApptOpen(false)} okText="提交预约">
        <p>选择看房时间（同时段每套房源仅一个有效预约）：</p>
        <input type="datetime-local" style={{ width: '100%', padding: 6 }} value={apptTime || ''}
          onChange={e => setApptTime(e.target.value)} />
        <Input.TextArea style={{ marginTop: 12 }} rows={2} maxLength={200} placeholder="留言（选填）"
          value={apptRemark} onChange={e => setApptRemark(e.target.value)} />
      </Modal>

      <Modal title="发起在线签约（FR-18）" open={contractOpen} onOk={submitContract} confirmLoading={contracting}
        onCancel={() => setContractOpen(false)} okText="生成合同">
      ...
      <Modal title="举报房源" open={reportOpen} onOk={submitReport} okText="提交举报" okButtonProps={{ danger: true }}
        onCancel={() => setReportOpen(false)}>
        <Input.TextArea rows={3} maxLength={200} placeholder="请描述举报理由（虚假房源、价格异常等）"
