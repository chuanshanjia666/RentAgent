...
import { fmtTime, NOTIFY_TYPE } from '../constants'
...
export default function NotificationsView() {
  ...
  return (
    <div className="page">
      ...
      <Tabs
        activeKey={tab}
        onChange={k => { setTab(k); setPage(1); load(1, k) }}
        items={[{ key: 'all', label: '全部' }, { key: 'unread', label: '未读' }]}
      />
      {list.map(n => (
        <div key={n.id} className={'notif' + (n.isRead ? '' : ' unread')} onClick={() => read(n)}>
          <Tag color={n.type === 2 || n.type === 5 ? 'orange' : 'blue'}>{NOTIFY_TYPE[n.type] || '消息'}</Tag>
          <div style={{ flex: 1 }}>
            <b>{n.title}</b>
