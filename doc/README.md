...
| `04.编码/` | 四、编码 | `单元测试用例设计.md`（后端 190 条 + 前端 26 条单元用例，含首轮缺陷记录 BUG-01）；代码位于仓库根目录 `frontend/`、`server/`、`docker/` 与 `docker-compose.yml`；前端一套代码两种分发（Web 版 + Electron 桌面端，见 `frontend/electron/`）；前端风格门禁为 ESLint + Prettier（`frontend/eslint.config.js`、`frontend/prettier.config.js`，`npm run lint` / `npm run format:check`） | 待补（编码阶段说明） |
| `05.集成测试/` | 五、集成测试 | `测试用例设计.md`（接口/集成/端到端 130 条断言 + FR-12 解析判分表 + 需求追溯矩阵）、`CI流水线设计.md`（三作业门禁与本地一致性）；可执行脚本 `scripts/ci-smoke.sh`、流水线 `.github/workflows/ci.yml` | 《集成测试报告》待补 |
