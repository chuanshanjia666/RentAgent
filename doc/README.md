...
| `04.编码/` | 四、编码 | `单元测试用例设计.md`（后端 148 条 + 前端 20 条单元用例，含首轮缺陷记录 BUG-01）；代码位于仓库根目录 `frontend/`、`server/`、`docker/` 与 `docker-compose.yml`；前端一套代码两种分发（Web 版 + Electron 桌面端，见 `frontend/electron/`） | 待补（编码阶段说明） |
