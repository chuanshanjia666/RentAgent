...
内容以《概要设计说明》v1.10、《数据库设计简介》v1.6、docker/mysql-init/01_schema.sql
与 server 端实际代码为唯一依据。
"""
...
CHANGE_ROWS = [
    ...
    ("7", "V1.6",
     ...
     "2026-09-18"),
    ("8", "V1.7",
     "找房智能体工具能力升级（需求基线不变）：① searchHouses 工具扩参——新增 minRent（预算下限）、"
     "facilities（逗号分隔设施标签）、sort（rent_asc / rent_desc），单次返回 5 → 8 套，"
     "返回体改为 {total, list[]}（模型可知房源总数），房源卡片补 coverUrl 与详情页链接 detailUrl"
     "（由 HousingTools 统一拼装 #/app/houses/{id}）；② getHouseDetail 工具实装（按 id 查单套房源"
     "完整信息，供模型回答追问）；③ 系统提示词约定房源链接协议：推荐逐套输出 [标题](detailUrl) "
     "Markdown 链接且原样引用工具返回值，不得编造房源与链接，空结果如实说明；④ 前端聊天气泡把回答中的"
     "站内链接渲染为可点击 <a>（历史会话回放天然携带）。SSE 契约与数据结构零变化；"
     "图 3-8 类图与 AI 模块结构图的 HousingTools 节点同步",
     "2026-09-19"),
...
REF_DOCS = [
    ...
    ("8", "概要设计说明", "王硕", "2026-09-19", "v1.11"),
    ("9", "数据库设计简介", "李天泽", "2026-09-18", "v1.6"),
...
MODULES_CONTENT = [
    ...
    dict(
        ...
        funcs=[
            ...
            ("7", "房源详情", "房东可查看自有全部状态房源；租客侧仅可见已上架房源；详情访问计为热度，"
                            "作为推荐热度因子（FR-11）——仅非房主、非管理员浏览时累加，"
                            "避免房东自查与管理端翻台账把房源热度刷高"),
