...
REF_DOCS = [
    ("1", "01-项目技术背景介绍", "项目组", "2026-09-03", "v1.0"),
    ("2", "02-项目最终确认书", "项目组", "2026-09-03", "v1.3"),
    ("3", "03-需求分析矩阵", "项目组", "2026-09-04", "v1.1"),
    ("4", "05-需求分析评审报告", "姜厚云 / 项目组", "2026-09-07", "v1.0"),
    ("5", "06-原型设计评审报告", "姜厚云 / 项目组", "2026-09-07", "v1.0"),
    ("6", "07-项目企划书", "项目组", "2026-09-13", "v1.0"),
    ("7", "08-需求跟踪矩阵", "项目组", "2026-09-07", "v1.2"),
    ("8", "概要设计说明", "王硕", "2026-09-15", "v1.3"),
    ("9", "数据库设计简介", "李天泽", "2026-09-14", "v1.1"),
...
SEC2_INTRO = [
    ...
    "与本地图片卷组成；大模型服务作为外部依赖经 HTTPS 出网调用，不可用时按降级链路回落到内置规则引擎。",
    "系统结构如图 2-1 所示。",
]
...
MODULES_CONTENT = [
    ...
    dict(
        ...
        title="AI 智能体服务模块",
        intro=[
            "AI 智能体服务模块是本项目的特色模块，覆盖 AI 找房助手、智能客服、合同智能解读、智能定价建议与"
            "虚假房源检测五项功能。模块在常规三层架构之下引入智能体层：由 AgentEngine 接口抽象编排能力，"
            "LlmAgent 基于 LangChain4j 的 AiServices 实现真实模型编排，MockAgent 作为无有效密钥时的降级实现，"
            "两者对上层完全透明（NFR-09 可插拔）。",
            ...
            "其中 anthropic-messages 与 openai-responses 的适配器为本项目自研实现，前者兼容兼容端点的思维链块"
        ...
        funcs=[
            ...
            ("4", "智能定价建议", "按“同小区 → 同区域同户型”两级取样本，输出租金区间、计算依据与样本量，"
                             "样本不足时明确提示（FR-15）"),
            ("5", "虚假房源检测", "对房源文本与图片做一致性启发式规则打分（0~100），输出风险分与疑点列表，"
                             "管理员保留最终裁决权（FR-16）"),
...
SPECS = [
    ...
    dict(module="房源管理模块", name="assistFill", file="AiController.java / AnalysisService.java",
         ...
         notes="该功能为 P2 增强项，不阻塞发布主流程；模型不可用时由规则引擎给出基础建议，不返回错误中断填写。"),
    ...
    dict(module="AI 智能体服务模块", name="suggestPrice", file="AiController.java / AnalysisService.java",
         ...
         notes="定价结果为参考建议而非定价约束，界面需明确标注样本量与依据；"
