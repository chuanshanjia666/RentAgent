...
> 文档版本：v1.6（v1.6 修订日期：2026-09-18，同步两处实现口径——house.facilities 在实体侧统一映射为数组、house.view_count 明确为“仅非房主/非管理员浏览才累加”；表结构与字段无变更；v1.5 修订日期：2026-09-18，viewing_appointment 增 active_slot 列、唯一键改 uk_active_slot，时段互斥只约束仍占时段的有效预约；v1.4 于 2026-09-17 同步 AI 去模拟化口径；见版本记录）
> 编写日期：2026-09-13（按课程进度计划，2026-09-15"概要设计说明以及数据库设计简介"节点交付评审）
> 编制：李天泽（文档 / 数据）　参与评审：项目组全体
> 关联文档：《需求分析矩阵》v1.1、《概要设计说明》v1.10、《需求跟踪矩阵》v1.1
...
| 设计结论 | `viewing_appointment` 表：tenant_id、house_id、landlord_id、appointment_time、status、active_slot……；`active_slot`（时段占位键）上的唯一索引 `uk_active_slot` 落实"禁止重复预约"验收标准——只约束"待确认/已确认"这类仍占时段的预约，终态置 NULL 释放时段 |
...
7. viewing_appointment（**id**，house_id*FK*，tenant_id*FK*，landlord_id*FK*，appointment_time，status，active_slot，reject_reason，remark，created_at，updated_at）
...
| view_count | INT UNSIGNED | DEFAULT 0 | 浏览数（FR-11 推荐热度因子）；**写入口径：仅非房主、非管理员浏览详情时累加**（房东自查与管理端翻台账不计入） |
...
| active_slot | VARCHAR(64) | NULL, UNIQUE | 时段占位键 `house_id:appointment_time`：仅待确认/已确认非空，终态置 NULL 释放时段 |
| reject_reason | VARCHAR(200) | | 拒绝理由 |
| remark | VARCHAR(200) | | 租客留言 |
| created_at / updated_at | DATETIME | NOT NULL | 审计字段 |
| 约束 | | UNIQUE(active_slot) | 同时段防重复预约（FR-17 验收标准）：只约束仍占时段的有效预约 |
...
| viewing_appointment | active_slot | 唯一 | 时段防冲突（FR-17）：键值 `house_id:appointment_time`，仅待确认/已确认持有，终态置 NULL 释放时段（唯一索引不对 NULL 去重） |
...
- **并发控制**：预约创建与合同签署用"唯一索引 + 事务"兜底幂等，状态更新一律 `WHERE status=期望前态`（乐观状态机），杜绝非法跳转。预约的时段互斥落在 `active_slot` 上：创建/确认时写入占位键，进入终态（已拒绝/已完成/已取消）时置 NULL——借"唯一索引不对 NULL 去重"的语义，让同一房源同一时段在终态后可被重新预约，而无需删除历史记录。
...
| v1.4 | 2026-09-17 | 同步"移除全部模拟数据、AI 全部调用真实模型"口径：`ai_analysis.model` 不再出现 `rule-engine` 取值，一律记录实际生效的 `协议:模型`；`ai_chat_message.citations` 的来源改由 `searchKnowledge` 工具返回体捕获（原规则引擎路径已删除）；表结构与字段无变更（仅口径说明） | 项目组全体 |
| v1.5 | 2026-09-18 | 按代码评审结论校正预约时段约束的落库方式：`viewing_appointment` 增 `active_slot VARCHAR(64) NULL`（时段占位键 `house_id:appointment_time`），唯一键由 `uk_house_time (house_id, appointment_time)` 改为 `uk_active_slot (active_slot)`。原口径下"已拒绝/已完成/已取消"的历史预约仍永久占用时段，用户无法重新预约同一时段；改为终态置 NULL 后释放时段，唯一键只约束待确认/已确认两类有效预约（MySQL 唯一索引不对 NULL 去重）。已同步建库脚本 `docker/mysql-init/01_schema.sql`（含列与唯一键变更、`idx_tenant`/`idx_landlord` 保留）与《数据库表》，并在第 4.3 节表 7、第 5.2 节索引设计、第 5.3 节并发控制中更新说明。**存量库需执行 ALTER 迁移**（`01_schema.sql` 只在数据卷首次初始化时执行）：① 加列并建 `uk_active_slot`；② 先补 `KEY fk_appt_house (house_id)` 再删 `uk_house_time`——外键 `fk_appt_house` 依赖该索引，直接删会报 `Cannot drop index ... needed in a foreign key constraint`（全新安装时 MySQL 会为此外键自动建同名索引，故迁移后表结构与全新安装逐项一致）；③ 按 `CONCAT(house_id, ':', 时段)` 回填仍占时段（待确认/已确认）的历史行，格式与 Java 端 `houseId + ":" + LocalDateTime` 一致（秒为 0 时省略秒），否则同一时段会因占位键不同而重复可约 | 项目组全体 |

| v1.6 | 2026-09-18 | 同步两处实现口径（**表结构与字段无变更**）：① `house.facilities`（JSON 列）在实体侧统一映射为 `List<String>`——MyBatis-Plus `@TableField(typeHandler = JacksonTypeHandler.class)` + `@TableName(autoResultMap = true)`，前后端不再兼容“JSON 字符串或数组”两种形状（原先入参用数组、出参给字符串、AI 填充接口又给数组，前端与冒烟脚本都得写类型分支）；**新增 JSON 列请照此办理**，且 JSON 列的读写统一走 `common/JsonColumns`（容错解析不抛异常）。② `house.view_count` 的写入口径明确为“仅非房主、非管理员浏览详情时累加”，填报在字段说明与第 4.4 节；推荐位按该列排序，原实现下房东自查与管理端审核都会把热度刷高。另同步《概要设计说明》引用版本至 v1.10 | 项目组全体 |
---
